...
use SimpleMVCProject\Form\Type\ArticleType;
use SimpleMVCProject\Domain\User;
use SimpleMVCProject\Form\Type\UserType;
        ...
        if ($app['security.authorization_checker']->isGranted(
            ...
            if ($commentForm->isSubmitted() && $commentForm->isValid()) {
                ...
                $app['session']->getFlashBag()
                    ->add('success', 'Your comment was successfully added.');
            }
...
$app->match('/admin/article/{id}/edit', 
    function ($id, Request $request) use ($app) {
        ...
        if ($articleForm->isSubmitted() && $articleForm->isValid()) {
            $app['dao.article']->save($article);
            $app['session']->getFlashBag()
                ->add('success', 'The article was successfully updated.');
        }
        return $app['twig']->render('article_form.html.twig', 
            array(
                'title' => 'Edit article',
...
$app->get('/admin/article/{id}/delete', 
    function ($id, Request $request) use ($app) {
        ...
        $app['session']->getFlashBag()
            ->add('success', 'The article was successfully removed.');
...
$app->match('/admin/comment/{id}/edit', 
    function ($id, Request $request) use ($app) {
        ...
        if ($commentForm->isSubmitted() && $commentForm->isValid()) {
            $app['dao.comment']->save($comment);
            $app['session']->getFlashBag()
                ->add('success', 'The comment was successfully updated.');
        }
...
$app->get('/admin/comment/{id}/delete', 
    function ($id, Request $request) use ($app) {
        ...
        $app['session']->getFlashBag()
            ->add('success', 'The comment was successfully removed.');
        // Redirect to admin home page
        return $app->redirect($app['url_generator']->generate('admin'));
    })
...
$app->match('/admin/user/{id}/edit', 
    function ($id, Request $request) use ($app) {
        ...
        if ($userForm->isSubmitted() && $userForm->isValid()) {
            ...
            $app['dao.user']->save($user);
            $app['session']->getFlashBag()
                ->add('success', 'The user was successfully updated');
        }
        return $app['twig']->render('user_form.html.twig', 
            array(
                'title' => 'Edit user',
...
$app->get('/admin/user/{id}/delete', 
    function ($id, Request $request) use ($app) {
        ...
        $app['session']->getFlashBag()
            ->add('success', 'The user was successfully removed');
        // Redirect to admin home page
        return $app->redirect($app['url_generator']->generate('admin'));
    })
